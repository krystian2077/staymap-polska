...
export default function HostProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:py-10">
      ...
      {/* Profile header card */}
      <div className="mb-6 overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm dark:bg-brand-surface">
        <div className="flex flex-col items-center gap-5 px-6 py-8 sm:flex-row sm:items-center sm:gap-7 sm:px-8">
          {/* Avatar */}
          <div className="group relative shrink-0 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
            <div className="h-24 w-24 overflow-hidden rounded-full border-2 border-brand-border shadow-md sm:h-28 sm:w-28">
              {avatarSrc ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={avatarSrc} alt="" className="h-full w-full object-cover" />
              ) : (
                <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-brand/20 to-brand/40 text-3xl font-extrabold text-brand-dark">
                  {initials}
                </div>
              )}
            </div>
          ...
          <div className="min-w-0 flex-1 text-center sm:text-left">
            <h1 className="truncate text-2xl font-extrabold text-brand-dark">{displayName}</h1>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              ...
              {memberSince && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-muted px-3 py-1 text-xs text-text-muted">
          ...
          <button
            ...
            className="shrink-0 self-start rounded-xl border border-brand-border bg-white px-4 py-2 text-xs font-semibold text-text-secondary shadow-sm transition hover:border-brand hover:text-brand disabled:opacity-60 dark:bg-brand-surface"
          >
      ...
      <form onSubmit={(e) => void handleSave(e)} className="space-y-5">
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm dark:bg-brand-surface">
          <div className="border-b border-brand-border/60 px-6 py-4 sm:px-8">
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm dark:bg-brand-surface">
          <div className="border-b border-brand-border/60 px-6 py-4 sm:px-8">
            ...
            <p className="mt-0.5 text-xs text-text-muted">Widoczny dla gości przeglądających Twoje oferty.</p>
          </div>
          <div className="px-6 py-5 sm:px-8">
            ...
            <div className="mt-2 flex justify-between">
              ...
              <span className={`text-[11px] tabular-nums ${bio.length > 7500 ? "text-orange-500 font-medium" : "text-text-muted"}`}>
                {bio.length}/8000
              </span>
            </div>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm dark:bg-brand-surface">
          ...
          <div className="divide-y divide-brand-border/40 px-6 sm:px-8">
            ...
            <div className="flex items-center justify-between py-3.5">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full bg-brand/10 px-2.5 py-0.5 text-xs font-semibold text-brand">Gospodarz</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end pt-1 pb-6">
          <button
            type="submit"
            disabled={saving}
            className="btn-primary flex min-w-[152px] items-center justify-center gap-2 px-7 py-2.5 text-sm disabled:opacity-70"
          >
            {saving ? (
              <>
