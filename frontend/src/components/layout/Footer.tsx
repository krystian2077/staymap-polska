...
export function Footer() {
  return (
    <footer
      ...
      className="relative overflow-hidden border-t border-[#e4ebe7] bg-[linear-gradient(180deg,#f6faf7_0%,#f8faf9_36%,#f8faf9_100%)] px-4 py-10 sm:px-6 sm:py-12 md:px-10 md:py-14 xl:px-12 dark:border-brand-border dark:bg-[linear-gradient(180deg,var(--bg)_0%,var(--bg2)_100%)]"
    >
      ...
      <div className="mx-auto w-full max-w-[1240px] rounded-[20px] border border-[#e4ebe7] bg-white/72 px-4 py-6 shadow-[0_10px_34px_rgba(10,15,13,.06)] backdrop-blur-md sm:rounded-[24px] sm:px-6 sm:py-7 md:px-8 md:py-8 dark:border-brand-border dark:bg-[var(--bg2)]/90 dark:shadow-[0_10px_34px_rgba(0,0,0,.3)]">
        <div className="grid gap-8 sm:gap-9 md:grid-cols-2 xl:grid-cols-[1.25fr_.85fr_.85fr]">
          <div>
            ...
            >
              <span className="text-[26px] font-black tracking-[-.9px] text-[#0a2e1a] sm:text-[30px] dark:text-[var(--foreground)]">StayMap</span>
              <span className="ml-0.5 text-[30px] leading-none text-[#16a34a] sm:text-[34px] dark:text-brand">.</span>
            </Link>
            <p className="mt-4 max-w-[420px] text-[15px] leading-7 text-[#3d4f45] sm:text-[16px] sm:leading-8 dark:text-[var(--text2)]">
              Premium noclegi w Polsce - od Mazur i Bałtyku po Tatry, z wyszukiwaniem wspieranym
              AI i profesjonalnym wsparciem gospodarzy.
            </p>
            <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-[#bbf7d0] bg-[#f0fdf4] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[.08em] text-[#166534] dark:border-brand-border dark:bg-[var(--gs)] dark:text-brand">
              Forest Premium Experience
            </div>
          </div>

          <div>
            <p className="text-[11px] font-extrabold uppercase tracking-[.14em] text-[#7a8f84] dark:text-[var(--text3)]">Platforma</p>
            <nav className="mt-3 flex flex-col gap-2" aria-label="Linki platformy">
              {PRODUCT_LINKS.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="group inline-flex w-fit items-center gap-2 rounded-[10px] px-2 py-1 text-[14px] font-semibold text-[#3d4f45] transition-all duration-300 ease-[cubic-bezier(.16,1,.3,1)] hover:-translate-y-0.5 hover:bg-[#f0fdf4] hover:text-[#0a2e1a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16a34a] focus-visible:ring-offset-2 focus-visible:ring-offset-white motion-reduce:hover:translate-y-0 dark:text-[var(--text2)] dark:hover:bg-[var(--gs)] dark:hover:text-[var(--foreground)]"
                >
          ...
          <div>
            <p className="text-[11px] font-extrabold uppercase tracking-[.14em] text-[#7a8f84] dark:text-[var(--text3)]">Informacje</p>
        ...
        <div className="mt-8 flex flex-col gap-3 border-t border-[#e4ebe7] pt-5 text-center text-xs text-[#7a8f84] sm:flex-row sm:items-center sm:justify-between sm:text-left dark:border-brand-border dark:text-[var(--text3)]">
